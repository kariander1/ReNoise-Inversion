...
epsilon_values = [0.3,0.5,1,2,3,5,10]  # Add more values as needed
...
for epsilon in epsilon_values:
        ...
        for idx, sample in enumerate(tqdm(ds, total=len(ds))):
            ...
            for edit_key, edit_info in ast.literal_eval(sample['edit_action']).items():
                ...
                x_0_hat_edit, inv_latent, noise_list, _, _, mean_inversions = run_model(
                    ...
                    noise=noise_list, edit_cfg=edit_cfg, do_reconstruction=True,inversion_step_type = "error_diff", 
                    epsilon=epsilon
                )
    ...
    combined_metrics_df.to_csv(f"./metrics_reports/edit_diff_error_epsilon_{epsilon}.csv", index=False)
    ...
    combined_metrics_df.to_csv(f"./metrics_reports/final_edit_diff_error_epsilon_{epsilon}.csv", index=False)
