...
epsilon_values = [1,10,20,25,30,40,45,50]  # Add more values as needed

# Iterate over epsilon values
for epsilon in epsilon_values:
    # DataFrame to store metrics
    metrics_list = []
    config_str = f"iter_error_epsilon{epsilon}"
    ...
    for idx, sample in enumerate(tqdm(ds, total=len(ds))):
        ...
        # Calculate and print running mean
        metrics_df = pd.DataFrame(metrics_list)
        running_mean_metrics = metrics_df.mean()
        print(f"Mean of each metric after {idx + 1} iterations for epsilon {epsilon}:")
    ...
    metrics_df.to_csv(f"./metrics_reports/final_{config_str}.csv", index=False)
